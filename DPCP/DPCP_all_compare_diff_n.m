...
F_val_soc_avg = zeros([test_number, N_in_num]);
F_val_madmm_avg = zeros([test_number, N_in_num]);
F_val_radmm_avg = zeros([test_number, N_in_num]);
F_val_nepv_avg = zeros([test_number, N_in_num]);

cpu_time_soc = zeros([test_number, N_in_num]); 
cpu_time_madmm = zeros([test_number, N_in_num]); 
cpu_time_radmm = zeros([test_number, N_in_num]); 
cpu_time_nepv = zeros([test_number, N_in_num]); 

error_X_nepv = zeros([test_number, N_in_num]);
error_X_radmm = zeros([test_number, N_in_num]);
error_X_madmm = zeros([test_number, N_in_num]);
error_X_soc = zeros([test_number, N_in_num]);
error_X_manppa = zeros([test_number, N_in_num]);
error_X_psgm = zeros([test_number, N_in_num]);

iter_nepv = zeros([test_number, N_in_num]);
iter_radmm = zeros([test_number, N_in_num]);
iter_madmm = zeros([test_number, N_in_num]);
iter_soc = zeros([test_number, N_in_num]);

Time_PSGM = zeros([test_number, N_in_num]); Fval_PSGM = zeros([test_number, N_in_num]);
Time_ManPPA = zeros([test_number, N_in_num]); Fval_ManPPA = zeros([test_number, N_in_num]);


for id_n = 1 : size(N_in_set, 2)
...
%% plot
figure(1);
plot(N_in_set, t_soc,    'r-s', 'MarkerSize', 10, 'LineWidth', 1); hold on;
plot(N_in_set, t_madmm,  'k-o', 'MarkerSize',  6, 'LineWidth', 1);
plot(N_in_set, t_radmm,  'b-d', 'MarkerSize',  8, 'LineWidth', 1);
plot(N_in_set, t_nepv,   'c-^', 'MarkerSize', 20, 'LineWidth', 1);
plot(N_in_set, t_psgm,   'm-*', 'MarkerSize', 20, 'LineWidth', 1);
plot(N_in_set, t_manppa, 'g-.', 'MarkerSize', 20, 'LineWidth', 1);
hold off;

xlabel('p_{1}', 'FontSize', 18);
ylabel('CPU',   'FontSize', 18);
legend('SOC','MADMM','RADMM','NEPvADMM','PSGM','ManPPA','FontSize', 11);

% --- adjust axis ranges here ---
%xlim([min(N_in_set)  max(N_in_set)]);   % or e.g. xlim([0 100])
ylim([-2  25]);                          % or e.g. ylim auto
% axis([0 100 0 50]);                   % alternative one-liner
% axis tight;                           % alternative: fit to data



figure(2)
semilogy(N_in_set, Fval_soc, 'r-s','MarkerSize',10,'linewidth',1); hold on;
semilogy(N_in_set, Fval_madmm, 'k-o','MarkerSize',6,'linewidth',1); hold on;
semilogy(N_in_set, Fval_radmm, 'b-d','MarkerSize',8,'linewidth',1); hold on;
semilogy(N_in_set, Fval_nepv, 'c-^','MarkerSize',20,'linewidth',1.5); hold on;
semilogy(N_in_set, Fval_ps, 'm-*','MarkerSize',20,'linewidth',1.5); hold on;
semilogy(N_in_set, Fval_man, 'g-.','MarkerSize',20,'linewidth',1.5);
legend('SOC', 'MADMM', 'RADMM', 'NEPvADMM', 'PSGM', 'ManPPA', 'FontSize', 11);
xlabel('p_{1}', 'FontSize', 18);   ylabel('objective fucntion value', 'FontSize', 18);

ylim([460  545]); 


figure(3)
plot(N_in_set, it_soc, 'r-s','MarkerSize',10,'linewidth',1); hold on;
plot(N_in_set, it_madmm, 'k-o','MarkerSize',6,'linewidth',1); hold on;
plot(N_in_set, it_radmm, 'c-^','MarkerSize',20,'linewidth',1.5); hold on;
plot(N_in_set, it_nepv, 'm-*','MarkerSize',20,'linewidth',1.5); hold on;
xlabel('p_{1}', 'FontSize', 18);   ylabel('iter', 'FontSize', 18);
...
figure(4)
plot(N_in_set, er_soc, 'r-s','MarkerSize',10,'linewidth',1); hold on;
plot(N_in_set, er_madmm, 'k-o','MarkerSize',6,'linewidth',1); hold on;
plot(N_in_set, er_radmm, 'c-^','MarkerSize',20,'linewidth',1.5); hold on;
plot(N_in_set, er_nepv, 'm-*','MarkerSize',20,'linewidth',1.5); hold on;
xlabel('p_{1}', 'FontSize', 18);   ylabel('error', 'FontSize', 18);
legend('SOC', 'MADMM', 'RADMM', 'NEPvADMM', 'FontSize', 11);
