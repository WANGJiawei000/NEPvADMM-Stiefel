...
for id_r = 1:size(r_set,1)
    ...
    iter.manpg(id_r) =  sum(maxit_att_manpg)/succ_no_manpg;
    iter.soc(id_r) =  sum(maxit_att_soc)/succ_no_SOC;
    iter.Rsub(id_r) =  sum(maxit_att_Rsub)/succ_no_sub;
    iter.madmm(id_r) =  sum(maxit_att_madmm)/succ_no_madmm;
    iter.radmm(id_r) =  sum(maxit_att_radmm)/succ_no_radmm;
    iter.nepv(id_r) =  sum(maxit_att_nepv)/succ_no_nepv;
    
    time.manpg(id_r) =  sum(time_manpg)/succ_no_manpg;
    time.soc(id_r) =  sum(time_soc)/succ_no_SOC;
    time.Rsub(id_r) =  sum(time_Rsub)/succ_no_sub;
    time.madmm(id_r) =  sum(time_madmm)/succ_no_madmm;
    time.radmm(id_r) =  sum(time_radmm)/succ_no_radmm;
    time.nepv(id_r) =  sum(time_nepv)/succ_no_nepv;
    
    Fval.manpg(id_r) =  sum(F_manpg)/succ_no_manpg;
    Fval.soc(id_r) =  sum(F_soc)/succ_no_SOC;
    Fval.Rsub(id_r) =  sum(F_Rsub)/succ_no_sub;
    Fval.madmm(id_r) =  sum(F_madmm)/succ_no_madmm;
    Fval.radmm(id_r) =  sum(F_radmm)/succ_no_radmm;
    Fval.nepv(id_r) =  sum(F_nepv)/succ_no_nepv;
    
    Sp.manpg(id_r) =  sum(sparsity_manpg)/succ_no_manpg;
    Sp.soc(id_r) =  sum(sparsity_soc)/succ_no_SOC;
    Sp.Rsub(id_r) =  sum(sparsity_Rsub)/succ_no_sub;
    Sp.madmm(id_r) =  sum(sparsity_madmm)/succ_no_madmm;
    Sp.radmm(id_r) =  sum(sparsity_radmm)/succ_no_radmm;
    Sp.nepv(id_r) =  sum(sparsity_nepv)/succ_no_nepv;
    ...
    fprintf(fid,print_format, iter.manpg(id_r), Fval.manpg(id_r), Sp.manpg(id_r), time.manpg(id_r), mean(manpg_error));
    print_format =  'SOC:        %1.3e  %1.5e    %1.2f      %3.2f    %1.3e\n';
    fprintf(fid,print_format, iter.soc(id_r) , Fval.soc(id_r), Sp.soc(id_r), time.soc(id_r), mean(soc_error_XPQ));
    print_format =  'Rsub:       %1.3e  %1.5e    %1.2f      %3.2f  \n';
    fprintf(fid,print_format,iter.Rsub(id_r) ,  Fval.Rsub(id_r) ,Sp.Rsub(id_r),time.Rsub(id_r));
    print_format =  'MADMM:        %1.3e  %1.5e    %1.2f      %3.2f    %1.3e\n';
    fprintf(fid,print_format,iter.madmm(id_r) , Fval.madmm(id_r), Sp.madmm(id_r) ,time.madmm(id_r), mean(error_madmm));
    print_format =  'RADMM:        %1.3e  %1.5e    %1.2f      %3.2f    %1.3e\n';
    fprintf(fid,print_format,iter.radmm(id_r) , Fval.radmm(id_r), Sp.radmm(id_r) ,time.radmm(id_r), mean(error_radmm));
    print_format =  'NEPvADMM:        %1.3e  %1.5e    %1.2f      %3.2f    %1.3e\n';
    fprintf(fid,print_format,iter.nepv(id_r) , Fval.nepv(id_r), Sp.nepv(id_r) ,time.nepv(id_r), mean(error_nepv));
end


%% plot
figure(1);
plot(r_set, time.manpg, 'r-s','MarkerSize',10,'linewidth',1); hold on;
plot(r_set, time.madmm,'k-o','MarkerSize',6,'linewidth',1); hold on;
plot(r_set, time.soc,'b-d','MarkerSize',8,'linewidth',1); hold on;
plot(r_set, time.radmm,'c-^','MarkerSize',20,'linewidth',1); hold on;
plot(r_set, time.nepv, 'm-*','MarkerSize',20,'linewidth',1); hold on;
%plot(r_set, time.Rsub,'g-.','MarkerSize',20,'linewidth',1);
xlabel('p', 'FontSize', 18);   ylabel('CPU', 'FontSize', 18);
...
figure(2)
semilogy(r_set, Fval.manpg, 'r-s','MarkerSize',10,'linewidth',1); hold on;
semilogy(r_set, Fval.madmm, 'k-o','MarkerSize',6,'linewidth',1); hold on;
semilogy(r_set, Fval.soc, 'b-d','MarkerSize',8,'linewidth',1); hold on;
semilogy(r_set, Fval.radmm, 'c-^','MarkerSize',20,'linewidth',1.5); hold on;
semilogy(r_set, Fval.nepv, 'm-*','MarkerSize',20,'linewidth',1.5); hold on;
%semilogy(r_set, Fval.Rsub, 'g-.','MarkerSize',20,'linewidth',1.5);
legend('ManPG','MADMM','SOC','RADMM','NEPvADMM', 'FontSize', 11);
xlabel('p', 'FontSize', 18);   ylabel('objective fucntion value', 'FontSize', 18);
...
figure(3)
plot(r_set, Sp.manpg, 'r-s','MarkerSize',10,'linewidth',1); hold on;
plot(r_set, Sp.madmm, 'k-o','MarkerSize',6,'linewidth',1); hold on;
plot(r_set, Sp.soc, 'b-d','MarkerSize',8,'linewidth',1); hold on;
plot(r_set, Sp.radmm, 'c-^','MarkerSize',20,'linewidth',1.5); hold on;
plot(r_set, Sp.nepv, 'm-*','MarkerSize',20,'linewidth',1.5); hold on;
%plot(r_set, Sp.Rsub, 'g-.','MarkerSize',20,'linewidth',1.5);
xlabel('p', 'FontSize', 18);   ylabel('sparsity', 'FontSize', 18);
legend('ManPG','MADMM','SOC','RADMM','NEPvADMM', 'FontSize', 11);
...
figure(4)
plot(r_set, iter.manpg, 'r-s','MarkerSize',10,'linewidth',1); hold on;
plot(r_set, iter.madmm, 'k-o','MarkerSize',6,'linewidth',1); hold on;
plot(r_set, iter.soc, 'b-d','MarkerSize',8,'linewidth',1); hold on;
plot(r_set, iter.radmm, 'c-^','MarkerSize',20,'linewidth',1.5); hold on;
plot(r_set, iter.nepv, 'm-*','MarkerSize',20,'linewidth',1.5); hold on;
%plot(r_set, iter.Rsub, 'g-.','MarkerSize',20,'linewidth',1.5);
xlabel('p', 'FontSize', 18);   ylabel('iter', 'FontSize', 18);
legend('ManPG','MADMM','SOC','RADMM','NEPvADMM', 'FontSize', 11);
