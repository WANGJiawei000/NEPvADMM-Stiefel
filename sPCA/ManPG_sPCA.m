function [X_manpg, F_manpg,sparsity,time_manpg,error_manpg,iter,flag_succ,num_linesearch,mean_ssn] = ManPG_sPCA(B,option)
...
for iter = 2:maxiter
    ...
    ngx = AX; % negative gradient       pgx=gx-X*xgx;  %projected gradient
    ...
    while F_trial >= F(iter-1)-0.5/t*alpha*normDsquared
        ...
        f_trial = -0.5*sum(sum(Z.*(AZ)));     F_trial = f_trial+ h(Z);
    end
    X = Z; AX = AZ;
    F(iter) = F_trial;
...
error_manpg = norm(X.'*X - eye(r), 'fro');
...
if iter == maxiter && sqrt(normDsquared)/t > 1e-1
    ...
    flag_succ = 1;
    sparsity = sum(sum(abs(X_manpg) <= 5e-5))/(n*r);
    F_manpg = F(iter-1);
    ...
    fprintf(1,print_format, iter-1,min(F), time_manpg,sparsity, inner_flag, sum(num_inner)/(iter-1) ,sqrt(normDsquared)/t,num_linesearch, error_manpg);
end